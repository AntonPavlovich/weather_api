...
export async function convertResponse(data) {
  ...
  if(daily ?? null) {
    response[Part.daily] = 
      daily?.map(day => getDataByKeys(day))
  }
  if(hourly ?? null) {
    response[Part.hourly] = 
      hourly?.map(hour => getDataByKeys(hour, [Keys.sunrise, Keys.sunset]))
  }
  if(minutely ?? null) {
    response[Part.minutely] = 
      minutely?.map(minute => getDataByKeys(minute, [Keys.sunrise, Keys.sunset]))
  }
...
function getDataByKeys(obj, exclude = [], keysArr = Object.values(Keys)) {
  return keysArr.reduce((acc, curr, i) => {
